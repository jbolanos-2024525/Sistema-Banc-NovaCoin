...
public class EmailService(IConfiguration configuration, ILogger<EmailService> logger) : IEmailService
{
    ...
    {
        var subject = "Verifica tu correo electrónico - NovaCoin";
        var verificationUrl = $"{configuration["AppSettings:FrontendUrl"]}/verify-email?token={token}";

        var body = $@"
            <h2>¡Bienvenido a NovaCoin, {username}!</h2>
            <p>Por favor, verifica tu correo electrónico para tu cuenta de NovaCoin haciendo clic en el siguiente enlace:</p>
    ...
    {
        var subject = "Restablece tu contraseña - NovaCoin";
        var resetUrl = $"{configuration["AppSettings:FrontendUrl"]}/reset-password?token={token}";

        var body = $@"
            <h2>Solicitud de restablecimiento de contraseña - NovaCoin</h2>
            <p>Hola {username},</p>
            <p>Este mensaje es de NovaCoin.</p>
    ...
    {
        var subject = "¡Bienvenido a NovaCoin!";
